...
if st.button("🚀 Gerar Prompt"):
    if all(entradas):
        ...
        prompt_seguro = prompt.replace("'", "\'")
        st.markdown(f'''<button onclick="navigator.clipboard.writeText('{prompt_seguro}')" style="padding:8px 16px; margin-top:10px;">📋 Copiar Prompt</button>''', unsafe_allow_html=True)
